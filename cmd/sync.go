...
var Sync = &cli.Command{
	Name:   "sync",
	Usage:  "Keeps syncing two end points",
	Action: syncCommand,
	Flags: []cli.Flag{
		&cli.StringFlag{Name: "mode", Value: "", Usage: "run mode"},
	},
}
...
func runServer(ctx *cli.Context) {
	if len(setting.Config.Sync.ListenAddr) == 0 {
		log.Fatal("Listen address cannot be empty")
	}

	l, err := net.Listen("tcp", setting.Config.Sync.ListenAddr)
	if err != nil {
		log.Fatal("Fail to start server on listening(%s): %v", setting.Config.Sync.ListenAddr, err)
	}

	log.Info("Listening on %s...", setting.Config.Sync.ListenAddr)
	for {
		conn, err := l.Accept()
		if err != nil {
...
func sendFile(fileName string) {
	...
	conn, err := net.Dial("tcp", setting.Config.Sync.RemoteAddr)
	if err != nil {
...
func runClient(ctx *cli.Context) {
	if len(setting.Config.Sync.RemoteAddr) == 0 {
